...
class ClientManager: ObservableObject {
    ...
    // Метод для сохранения данных о цветах в UserDefaults
        func saveCellColors() {
            let encoder = JSONEncoder()
        ...
        // Метод для загрузки данных о цветах из UserDefaults
        func loadCellColors() {
    ...
    func removeEmployee(_ employee: Employee) {
            if let index = employees.firstIndex(of: employee) {
                ...
                saveEmployeesData() // Сохраняем обновленные данные после удаления
            }
        }
